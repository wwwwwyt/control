...
protected:
  ...
  virtual void rightSwitchMidRise() //右中
  {
    state_ = RC;
  }
  virtual void rightSwitchMidOn() // 右中 位置处于高电平时调用的回调函数。
  {
    state_ = RC;
  }
  virtual void rightSwitchUpRise() //右上
  {
    state_ = PC;
  }
  virtual void rightSwitchUpOn() // 右上 位置处于高电平时调用的回调函数。
  {
    state_ = PC;
  }
